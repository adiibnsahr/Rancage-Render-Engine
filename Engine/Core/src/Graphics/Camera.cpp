...
#include <Windows.h>
#include <cmath>
#include <algorithm> // Tambah untuk std::max dan std::min

namespace Graphics
{
    Camera::Camera()
        : m_Position(0.0f, 0.0f, 5.0f) // Mulai di Z=2
    ...
    void Camera::Update(float deltaTime)
    {
        ...
        float clampedPitch = m_Rotation.x;
        if (clampedPitch > 1.5708f) clampedPitch = 1.5708f;
        if (clampedPitch < -1.5708f) clampedPitch = -1.5708f;
        m_Rotation.x = clampedPitch; // Clamp pitch
