...
namespace Graphics
{
    ...
    bool Device::Initialize(HWND hwnd, UINT width, UINT height)
    {
        ...
        // Create constant buffer
        {
            ...
            CD3DX12_RESOURCE_DESC bufferDesc = CD3DX12_RESOURCE_DESC::Buffer(1024 * 64); // 64KB aligned
    ...
    void Device::UpdateConstantBuffer(const Math::Matrix4& modelMatrix)
    {
        // View matrix (kamera di z=2, lihat ke -Z)
        Math::Vector3 eye(0.0f, 0.0f, 2.0f);
        Math::Vector3 target(0.0f, 0.0f, 0.0f);
        Math::Vector3 up(0.0f, 1.0f, 0.0f);
        Math::Matrix4 view = Math::Matrix4::CreateLookAt(eye, target, up);

        // Projection matrix (FOV 60°, aspect 1280/720, near 0.1, far 100)
        Math::Matrix4 proj = Math::Matrix4::CreatePerspective(
            60.0f * 3.1415926535f / 180.0f, // FOV ke radian
            1280.0f / 720.0f,
            0.1f,
            100.0f
        );
